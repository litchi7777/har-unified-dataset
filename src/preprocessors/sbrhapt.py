"""
SBRHAPT (Smartphone-Based Recognition of Human Activities and Postural Transitions) データセット前処理

SBRHAPT データセット:
- 30名の被験者（19-48歳）
- 12種類の活動: 6基本活動 + 6姿勢遷移
  - 基本活動: Standing, Sitting, Lying, Walking, Walking Downstairs, Walking Upstairs
  - 姿勢遷移: Stand-to-Sit, Sit-to-Stand, Sit-to-Lie, Lie-to-Sit, Stand-to-Lie, Lie-to-Stand
- 腰装着スマートフォン（Samsung Galaxy S II）
- 6軸データ: ACC(3) + GYRO(3)
- サンプリングレート: 50Hz
- 参照: https://archive.ics.uci.edu/dataset/341
"""
...
class SBRHAPTPreprocessor(BasePreprocessor):
    """
    SBRHAPT (Smartphone-Based Recognition of Human Activities and Postural Transitions) データセット用の前処理クラス
    """
    ...
    def download_dataset(self) -> None:
        ...
        if target_dir.exists():
            ...
            if len(acc_files) >= 60 and len(gyro_files) >= 60:  # 最低限のファイル数
    ...
    def load_raw_data(self) -> Dict[int, Tuple[np.ndarray, np.ndarray]]:
        ...
        for acc_file in acc_files:
            # ファイル名から exp_id と user_id を取得
            # ファイル名形式: acc_exp01_user01.txt
        ...
        # 同一被験者の複数実験を時系列で結合
        all_data = {}
        for user_id, file_list in subject_data_list.items():
            # データとラベルを時系列で結合
            all_user_data = np.vstack([d for d, l in file_list])
            all_user_labels = np.concatenate([l for d, l in file_list])

            all_data[user_id] = (all_user_data, all_user_labels)
            logger.info(f"USER{user_id:05d}: combined data={all_user_data.shape}, "
                       f"labels={all_user_labels.shape}, unique={np.unique(all_user_labels)}")
    ...
    def clean_data(
        ...
        for user_id, (user_data, labels) in data.items():
            # NaN除去
            valid_mask = ~np.isnan(user_data).any(axis=1)
            cleaned_data = user_data[valid_mask]
            ...
            if self.original_sampling_rate != self.target_sampling_rate:
                ...
                cleaned[user_id] = (resampled_data, resampled_labels)
                logger.info(f"USER{user_id:05d} resampled: {resampled_data.shape}")
            else:
                cleaned[user_id] = (cleaned_data, cleaned_labels)
                logger.info(f"USER{user_id:05d} cleaned: {cleaned_data.shape}")
    ...
    def extract_features(
        ...
        for user_id, (user_data, labels) in data.items():
            logger.info(f"Processing USER{user_id:05d}")
            processed[user_id] = {}

            # スライディングウィンドウ適用（全6チャンネル一括）
